...
import pprint
import json
import math # used log(x) as length
...
class FilesHandler:
    ...
    def __init__(self, settings_location, ui):
        ...
        эта ф-ция успевает 1 раз обратиться к ui"""
    ...
    def get_directory_content(self, is_first):
        ...
        нет никакой обработки перебоев связи"""
    ...
    def mainloop_cycle(self):
        ...
        if files_to_append:
            ...
            if not self.paused and time.time() - self.last_update_time['history'] > self.settings['seconds-for-overload']:
                self.pause()
